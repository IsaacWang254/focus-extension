...
import { TODOIST_CLIENT_ID, TOKEN_PROXY_URL } from './config.js';
import { getCachedResource, cacheScope } from './request-cache.js';

// =============================================================================
// CONFIGURATION
// OAuth client_secret is stored server-side in the Cloudflare Worker proxy.
// Only the access token is stored locally after authentication.
// =============================================================================

const TODOIST_AUTH_URL = 'https://todoist.com/oauth/authorize';
const TODOIST_API_BASE = 'https://api.todoist.com/api/v1';

const TODOIST_CACHE_KEYS = ['focusCache:todoist:tasks', 'focusCache:todoist:completedToday', 'focusCache:todoist:labels', 'focusCache:todoist:projects'];
const TASK_CACHE_TTL = 2 * 60 * 1000;
...
export async function authenticate() {
  ...
  try {
    ...
    await chrome.storage.local.remove(TODOIST_CACHE_KEYS);

    // Store the token
    await chrome.storage.local.set({ todoistToken: token, todoistCacheRevision: crypto.randomUUID() });
 ...
 * The proxy holds the client_secret — it never touches the client.
...
export async function logout() {
  await chrome.storage.local.remove(['todoistToken', ...TODOIST_CACHE_KEYS]);
  await chrome.storage.local.set({ todoistCacheRevision: crypto.randomUUID() });
}
...
async function apiRequest(endpoint, options = {}) {
  ...
  const { forceRefresh = false, staleWhileRevalidate = false, ...fetchOptions } = options;
  const method = (fetchOptions.method || 'GET').toUpperCase();
  const resource = /^\/(tasks|labels|projects)(?:\?|$)/.exec(endpoint)?.[1];
  if (method === 'GET' && resource) {
    const { todoistCacheRevision = '' } = await chrome.storage.local.get('todoistCacheRevision');
    const revision = resource === 'tasks' ? todoistCacheRevision : '';
    const scope = await cacheScope([token, revision, endpoint]);
    return getCachedResource(`focusCache:todoist:${resource}`, {
      scope, ttl: resource === 'tasks' ? TASK_CACHE_TTL : METADATA_CACHE_TTL,
      maxStale: staleWhileRevalidate ? 15 * 60 * 1000 : 0, staleWhileRevalidate, force: forceRefresh,
      isCurrent: async () => {
        const current = await chrome.storage.local.get(['todoistToken', 'todoistCacheRevision']);
        return current.todoistToken === token && (resource !== 'tasks' || (current.todoistCacheRevision || '') === revision);
      },
      load: () => requestWithToken(endpoint, fetchOptions, token)
    });
  }
  const result = await requestWithToken(endpoint, fetchOptions, token);
  if (method !== 'GET' && await getToken() === token) {
    await chrome.storage.local.set({ todoistCacheRevision: crypto.randomUUID() });
    await chrome.storage.local.remove(TODOIST_CACHE_KEYS.slice(0, 2));
  }
  return result;
}

async function requestWithToken(endpoint, options, token) {
  ...
  if (!response.ok) {
    const error = new Error(`Todoist API request failed: ${response.status}`);
...
/**
 * Fetch completed tasks by completion date
 * @param {object} options - Optional filters
 * @param {string} options.since - ISO 8601 datetime (e.g., '2026-02-09T00:00:00')
 * @param {string} options.until - ISO 8601 datetime (e.g., '2026-02-10T00:00:00')
...
export async function getCompletedTasks(options = {}) {
  ...
  if (options.until) {
    ...
    // API requires 'until' — default to now
    params.set('until', new Date().toISOString());
  }
  if (options.limit) params.set('limit', options.limit.toString());

  const queryString = params.toString();
  const endpoint = `/tasks/completed/by_completion_date${queryString ? `?${queryString}` : ''}`;
  ...
  console.warn('Unexpected completed tasks response format:', response);
  return [];
}

export async function getCompletedTasksToday({ limit = 50, forceRefresh = false, staleWhileRevalidate = false } = {}) {
  const token = await getToken();
  if (!token) {
    throw new Error('Not authenticated with Todoist');
  }
  ...
  const { todoistCacheRevision = '' } = await chrome.storage.local.get('todoistCacheRevision');
  const scope = await cacheScope([token, todoistCacheRevision, since, limit]);

  return getCachedResource('focusCache:todoist:completedToday', {
    scope,
    ttl: TASK_CACHE_TTL,
    maxStale: staleWhileRevalidate ? 15 * 60 * 1000 : 0,
    staleWhileRevalidate,
    force: forceRefresh,
    isCurrent: async () => {
      const current = await chrome.storage.local.get(['todoistToken', 'todoistCacheRevision']);
      return current.todoistToken === token && (current.todoistCacheRevision || '') === todoistCacheRevision;
    },
    load: () => getCompletedTasks({ since, until: new Date().toISOString(), limit })
  });
}
...
export async function getLabelsMap() {
  ...
  if (labelsCache && labelsCacheToken === token && (now - labelsCacheTime) < LABELS_CACHE_TTL) {
    return labelsCache;
  }
...
export function formatDueDate(task) {
  ...
  if (due.datetime) {
    ...
    // API v1 embeds the full timestamp in `date` for timed tasks; only
    // whole-day tasks use a bare YYYY-MM-DD value.
    hasTime = due.date.includes('T');
    dueDate = hasTime ? new Date(due.date) : new Date(due.date + 'T00:00:00');
  } else {
    return due.string || '';
  }
